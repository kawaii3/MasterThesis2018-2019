...
library(rgdal)
library(dplyr)
library(sirad) #extraterrestrial radiation
...
observations_total = read.xlsx("Data/commonbean.xlsx", 1)
observations = select(observations_total,lon, lat)
...
# a  = matrix(nrow =  length(observations$lat) , ncol =length(unique(temp_daily_df$DOY)))
...
#   for (j in 1:dim(d)[2]){  
#     day = temp_daily_df$DOY[j]
#     extraTerrestrial = extrat(day, lat)[[1]]
#     d[i, j] = extraTerrestrial
#     } 
# }
# 
# write.csv(d, "extraterristrial.csv")

### extraterristrial output from last step need to be modified in excel
#setwd(homefolder)
ra <- read.csv("Data/extraterristrial.csv")
...
tricot_data = select(observations_total, day, month, year)
...
#write.csv(waterbalance, "Data/waterbalance.csv")
